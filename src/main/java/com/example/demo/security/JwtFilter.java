    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            try {
                ...
                // Wewnątrz doFilterInternal
                String rolesClaim = jwtService.extractRoles(token); // np. "USER,ADMIN"

                List<SimpleGrantedAuthority> authorities = Arrays.stream(rolesClaim.split(","))
                        .map(role -> role.startsWith("ROLE_") ? role : "ROLE_" + role) // Dodaj prefiks jeśli go nie ma
